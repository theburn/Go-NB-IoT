...
func (l *logger) SetMaxLogDay(d int) {
	if d <= 0 {
		d = 7
	}
...
func (l *logger) deleteOldLog() {
	...
	filepath.Walk(dir, func(path string, info os.FileInfo, err error) (returnErr error) {
		...
		if !info.IsDir() && info.ModTime().Add(24*time.Hour*time.Duration(l.maxLogDay)).Before(time.Now()) {
			if strings.HasPrefix(filepath.Base(path), "rtmpproxy") {
				os.Remove(path)
			}
		}
